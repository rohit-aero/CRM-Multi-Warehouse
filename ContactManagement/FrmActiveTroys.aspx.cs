...
/// </summary>
public partial class ContactManagement_FrmActiveTroys : System.Web.UI.Page
{
    ...
    private void Bind_Controls()
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                // Utility.BindDropDownList(ddlBranchLocation, ds.Tables[0]);
            }
            ...
            {
                Utility.BindDropDownList(ddlBranchMain, ds.Tables[6]);
            }
        }
        catch (Exception ex)
        {
    ...
    protected void ddlBranch_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                hfCusId.Value = ddlTroyEmployee.SelectedValue;
                DataSet ds = new DataSet();
                ObjBOL.Operation = 2;
                ObjBOL.BranchID = Convert.ToInt32(ddlTroyEmployee.SelectedValue);
                ds = ObjBLL.GetRepBranches(ObjBOL);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    txtBrachCompany.Text= Convert.ToString(ds.Tables[0].Rows[0]["BranchName"]);
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (ValidationCheck() == true)
            {
                ...
                Utility.MaintainLogs("FrmActiveTroys.aspx", "Save");
                Reset();
            }
        }
        catch (Exception ex)
