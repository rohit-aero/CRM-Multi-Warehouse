    ...
    protected void gvExportToExcel_DataBound(object sender, EventArgs e)
    {
        for (int i = gvExportToExcel.Rows.Count - 1; i > 0; i--)
        {
            ...
            for (int j = 0; j < row.Cells.Count; j++)
            {
                ...
                gvExportToExcel.Rows[i].Cells[0].Style.Add("vertical-align", "top");
                gvExportToExcel.Rows[i].Cells[0].Style.Add("font-weight", "bold");
                gvExportToExcel.Rows[i].Cells[0].Style.Add("font-size", "25px");

                gvExportToExcel.Rows[i].Cells[1].Style.Add("text-align", "left");
            }
            if (gvExportToExcel.Rows.Count > 0)
            {
                gvExportToExcel.Rows[0].Cells[0].Style.Add("vertical-align", "top");
                gvExportToExcel.Rows[0].Cells[0].Style.Add("font-weight", "bold");
                gvExportToExcel.Rows[0].Cells[0].Style.Add("font-size", "25px");

                gvExportToExcel.Rows[0].Cells[1].Style.Add("text-align", "left");
            }
        }
    }
    private void ExportToCrystalData()
    {
        try
        {
            DataTable dt = new DataTable();
            var query = "EXEC aero_ManageForecasting 2, '" + txtFromDate.Text + "', '" + txtToDate.Text + "', " + ddlProduct.SelectedValue + ", " + ddlShop.SelectedValue;
            clscon.Return_DT(dt, query);
            dt.Columns.Remove(dt.Columns[0]);
            gvExportToExcel.DataSource = dt;
            gvExportToExcel.DataBind();
            //rprt.Load(Server.MapPath("~/Reports/rptSummaryForecastingNew.rpt"));
            //if (dt.Rows.Count > 0)
            //{
            //    TextObject txtheader = (TextObject)rprt.ReportDefinition.ReportObjects["txtHeader"];
            //    txtheader.Text = "Forecasting From " + txtFromDate.Text + " to " + txtToDate.Text;
            ...
            Utility.ExportToExcelGrid(gvExportToExcel, "Forecasting From " + txtFromDate.Text + " to " + txtToDate.Text);
            gvExportToExcel.DataSource = string.Empty;
            gvExportToExcel.DataBind();
            //Utility.ExportToExcelDT_Merge(dt, "Forecasting From " + txtFromDate.Text + " to " + txtToDate.Text, 0);
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
