...
public partial class ContactManagement_FrmReps_Troys : System.Web.UI.Page
{
    ...
    private void Bind_Control()
    {
        try
        {
            ...
            //{
            //    Utility.BindDropDownList(ddlSalesRepGroup, ds.Tables[8]);
            //}
            if (ds.Tables[9].Rows.Count > 0)
            {
                Utility.BindDropDownList(ddlProductLineList, ds.Tables[9]);
            }
        }
        catch (Exception ex)
        {
    ...
    private Boolean ValidationCheck()
    {
        try
        {
            ...
            {
                //ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "window", "alert('Please Select Branch. !');", true);
                Utility.ShowMessage_Error(Page, "Please Select Branch. !");
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        try
        {
            ddlProductLineList.SelectedIndex = 0;
            Reset();
        }
        catch (Exception ex)
        {
    ...
    private void SalesRepGroupEvent()
    {
        try
        {
    ...
    private void ProductLineListEvent()
    {
        try
        {
            ...
            if (ddlProductLineList.SelectedIndex > 0)
            {
                DataSet ds = new DataSet();
                ObjBOL.Operation = 8;
                ObjBOL.RepID = Int32.Parse(ddlProductLineList.SelectedValue);
                ds = ObjBLL.GetRepsDetail(ObjBOL);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    Utility.BindDropDownList(ddlSalesRepGroup, ds.Tables[0]);
