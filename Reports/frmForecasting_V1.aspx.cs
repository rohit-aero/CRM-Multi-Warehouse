...
public partial class Reports_frmForecasting_V1 : System.Web.UI.Page
{
    ...
    private DataTable ReportData()
    {
        ...
        try
        {
            if (ValidationCheck() == true)
            {
                ...
                Qstr += " DECLARE @Columns NVARCHAR(MAX)='' DECLARE @ColumnsSum NVARCHAR(MAX)='' DECLARE @SqlStatement NVARCHAR(MAX)='' ";

                Qstr += " SELECT @Columns += QUOTENAME( UPPER(CONVERT(VARCHAR, MIN(tblProjects.ShipDate), 101)) + ', ' ";
                Qstr += "  + MIN(Inv_AWProduct_Projects.JobID) + ', ' + ISNULL(MIN(tblCustomers.CompanyName), '') + ', ' ";
                Qstr += " + ISNULL(MIN(tblCustomers.City), '') + ', '  + ISNULL(MIN(tblStates.[State]), '')) + ', ' ";
                Qstr += " FROM Inv_Parts LEFT JOIN Inv_AWProduct_Parts     ON CASE WHEN ISNULL(Inv_AWProduct_Parts.ChildPartID, 0) = 0 ";
                Qstr += " THEN Inv_AWProduct_Parts.ParentPartID     ELSE Inv_AWProduct_Parts.ChildPartID END = Inv_Parts.ID ";
                ...
                Qstr += " GROUP BY UPPER( Inv_AWProduct_Projects.JobID + ', '  + ISNULL(tblCustomers.CompanyName, '') + ', ' ";
                Qstr += " + ISNULL(tblCustomers.City, '') + ', '   + ISNULL(tblStates.[State], '')) ";
                Qstr += " IF(@Columns <> '') BEGIN  SET @Columns = LEFT(@Columns, LEN(@Columns) - 1);  END ";

                Qstr += "  SELECT @ColumnsSum += 'SUM(' + QUOTENAME( UPPER(CONVERT(VARCHAR, MIN(tblProjects.ShipDate), 101)) + ', ' ";
                Qstr += "  + MIN(Inv_AWProduct_Projects.JobID) + ', ' + ISNULL(MIN(tblCustomers.CompanyName), '') + ', ' ";
                Qstr += "  + ISNULL(MIN(tblCustomers.City), '') + ', ' + ISNULL(MIN(tblStates.[State]), ''))  + ') AS ' ";
                Qstr += " + QUOTENAME( UPPER(CONVERT(VARCHAR, MIN(tblProjects.ShipDate), 101)) + ', '  + MIN(Inv_AWProduct_Projects.JobID) + ', ' ";
                Qstr += " + ISNULL(MIN(tblCustomers.CompanyName), '') + ', '  + ISNULL(MIN(tblCustomers.City), '') + ', ' ";
                Qstr += "  + ISNULL(MIN(tblStates.[State]), '')) + ', ' FROM Inv_Parts";
                Qstr += " LEFT JOIN Inv_AWProduct_Parts ON CASE WHEN ISNULL(Inv_AWProduct_Parts.ChildPartID, 0) = 0 ";
                Qstr += " THEN Inv_AWProduct_Parts.ParentPartID ELSE Inv_AWProduct_Parts.ChildPartID  END = Inv_Parts.id";
                Qstr += " INNER JOIN Inv_AWProduct_Sub ON Inv_AWProduct_Sub.ID = Inv_AWProduct_Parts.AWProductSubID ";
                Qstr += " INNER JOIN Inv_AWProduct ON Inv_AWProduct.ID = Inv_AWProduct_Sub.AWProductID ";
                Qstr += " LEFT JOIN Inv_AWProduct_Projects ON Inv_AWProduct_Projects.AWProductPartsID = Inv_AWProduct_Parts.id ";
                Qstr += " LEFT JOIN tblProjects ON Inv_AWProduct_Projects.JobID = tblProjects.JobID ";
                Qstr += " LEFT JOIN tblCustomers  ON tblCustomers.CustomerID = tblProjects.CustomerID ";
                Qstr += " LEFT JOIN tblStates ON tblStates.StateID = tblCustomers.StateID WHERE Inv_AWProduct_Projects.qty IS NOT NULL ";
                Qstr += "  AND tblProjects.ProjectStatus NOT IN(2, 3)  AND tblProjects.ShipDate BETWEEN '" + strDateFrom + "' AND '" + strDateTo + "' ";
                Qstr += " GROUP BY UPPER( Inv_AWProduct_Projects.JobID + ', ' + ISNULL(tblCustomers.CompanyName, '') + ', ' ";
                Qstr += " + ISNULL(tblCustomers.City, '') + ', ' + ISNULL(tblStates.[State], '')); ";
                Qstr += " IF(@ColumnsSum <> '') BEGIN SET @ColumnsSum = LEFT(@ColumnsSum, LEN(@ColumnsSum) - 1); END ";

                Qstr += " IF(@Columns <> '') BEGIN SET @SqlStatement = 'WITH PreAggregatedData AS( ";
                Qstr += " Select MIN(Inv_AWProduct.name) AS Product, Inv_Parts.PartNumber +'' ''+ ISNULL(Inv_Parts.revisionno,'''') ";
                Qstr += " + CASE WHEN PartStatus = 1 THEN '' - Current'' WHEN PartStatus = 2 THEN '' - Obsolete'' WHEN PartStatus = 3 THEN '' - Not In Use'' END ";
                Qstr += " AS [Part Number], MIN(Inv_Parts.PartDes) AS [Part Des], ";
                Qstr += " MIN(Inv_Parts.PartImage) AS [Part Image__________], ";
                Qstr += " MIN(UPPER(CONVERT(VARCHAR, tblProjects.ShipDate, 101) + '', '' + Inv_AWProduct_Projects.JOBID + '', '' ";
                Qstr += " + ISNULL(tblCustomers.CompanyName, '''') + '', '' + ISNULL(tblCustomers.City, '''') + '', '' ";
                Qstr += "  + ISNULL(tblStates.[State], ''''))) AS [Project_Name], ";
                Qstr += " MIN(Inv_Parts.[min]) AS [Min Qty],MIN(Inv_Parts.max) AS [Max Qty], CAST(SUM(Inv_AWProduct_Projects.qty) AS INT) as Qty, Inv_Parts.stockinhand AS [Opening Stock] ";
                Qstr += " FROM Inv_Parts  ";
                ...
                Qstr += " GROUP BY  Inv_Parts.PartNumber +'' ''+ ISNULL(Inv_Parts.revisionno,'''') ";
                Qstr += " + CASE WHEN PartStatus = 1 THEN '' - Current'' WHEN PartStatus = 2 THEN '' - Obsolete'' ";
                Qstr += " WHEN PartStatus = 3 THEN '' - Not In Use'' END, ";
                Qstr += "  Inv_Parts.stockinhand, CONVERT(VARCHAR, tblProjects.ShipDate, 101)) ";
                Qstr += "  SELECT MIN(Product) AS[Product], [Part Number],[Part Des], MIN([Part Image__________]) AS[Part Image__________], ";
                Qstr += " MIN([Min Qty]) AS [Min Qty], MIN([Max Qty]) AS [Max Qty], MIN([Opening Stock]) AS [Opening Stock], ";
                Qstr += " ' + @ColumnsSum +' FROM PreAggregatedData PIVOT( SUM([Qty]) FOR [Project_Name] IN (' +  @Columns + ') ";
                Qstr += " ) AS PivotTable GROUP BY [Part Number], [Part Des] Order by MIN(Product),[Part Number],[Part Des] asc' END ";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    DataColumn Col = dt.Columns.Add("Total Quantity Required", typeof(Int32));
    ...
    private DataTable ReportData_Summary()
    {
        ...
        try
        {
            ...
            query = "EXEC Get_Forecast_Summary_V2 1,'" + txtFromDate.Text + "', '" + txtToDate.Text + "', ";
            if (ddlProduct.SelectedIndex > 0)
            {
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                var columnNames = dt.Columns.Cast<DataColumn>()
                                .Where((c, index) => index != 3) // Exclude the 4th column (index 3)
                                .Take(8) // Take the first 9 columns after excluding the 4th
                ...
                gvExportToPDF.DataSource = newDt;
                gvExportToPDF.DataBind();
                if (BindHeader)
                {
    ...
    private void GenrateReport_SecondExcel()
    {
        try
        {
            ...
            if (gvSearch.Rows.Count > 0)
            {
                ...
                string FileName = "Forecasing " + ddlReportType.SelectedItem.Text + " " + DateTime.Now.ToString("MM/dd/yyyy");
    ...
    protected void gvSearch_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (!Int32.TryParse(temp, out field1))
                {
                    field1 = 0;
                }
            }
    ...
    protected void btnExporttoPdf_Click(object sender, EventArgs e)
    {
        try
        {
            gvExportToPDF.AllowPaging = false;
            this.BindGrid(true);
            gvExportToPDF.HeaderRow.BackColor = Color.White;

            // Set header row background color
            foreach (TableCell cell in gvExportToPDF.HeaderRow.Cells)
            {
                cell.BackColor = gvExportToPDF.HeaderStyle.BackColor;
            }
            ...
            using (StringWriter sw = new StringWriter())
            {
                ...
                {
                    gvExportToPDF.RenderControl(hw);
    ...
    private DataTable ExportDatatoExcel()
    {
        ...
        try
        {
            if (ValidationCheck() == true)
            {
                ...
                Qstr += " WHEN PartStatus = 3 THEN '' - Not In Use'' END, ";
                Qstr += "  Inv_Parts.stockinhand, CONVERT(VARCHAR, tblProjects.ShipDate, 101)) ";
                Qstr += "  SELECT MIN(Product) AS[Product], [Part Number],[Part Des],  ";
                Qstr += " MIN([Min Qty]) AS [Min Qty], MIN([Max Qty]) AS [Max Qty], MIN([Opening Stock]) AS [Opening Stock], ";
                Qstr += " ' + @ColumnsSum +' FROM PreAggregatedData PIVOT( SUM([Qty]) FOR [Project_Name] IN (' +  @Columns + ') ";
                Qstr += " ) AS PivotTable GROUP BY [Part Number], [Part Des] Order by MIN(Product),[Part Number],[Part Des] asc' END ";
                Qstr += " EXEC sp_executesql @SqlStatement; ";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    Col.SetOrdinal(5);// to put the column in position 0;
                    //dt.Columns.Add("Category", typeof(int));
                    DataColumn Col1 = dt.Columns.Add("Closing Stock", typeof(int));
                    Col1.SetOrdinal(6);
                    //dt.Columns.Add(" ", typeof(string));
    ...
    private DataTable ReportData_ExcelSummary()
    {
        ...
        {
            string query = string.Empty;
            query = "EXEC Get_Forecast_Summary_V2_Excel '" + txtFromDate.Text + "', '" + txtToDate.Text + "', ";
            if (ddlProduct.SelectedIndex > 0)
            {
                query += ddlProduct.SelectedValue + " ";
            }
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (BindHeader)
                {
                    ...
                    additionalHeaderCell.ColumnSpan = dt.Columns.Count;
                    additionalHeaderRow.Cells.Add(additionalHeaderCell);
                    gvSummary.Controls[0].Controls.AddAt(0, additionalHeaderRow);
                }
                //gvSearch.Columns[0].Visible = false;
            }
            else
    ...
    protected void gvSummary_DataBound(object sender, EventArgs e)
    {
        try
        {
            ...
            for (int i = gvSummary.Rows.Count - 1; i > 0; i--)
            {
                ...
                gvSummary.Rows[i].Cells[0].Style.Add("width", maxWidthForFirstColumn);
                gvSummary.Rows[i].Cells[0].HorizontalAlign = HorizontalAlign.Left;
                gvSummary.Rows[i].Cells[1].Style.Add("width", maxWidthForSecondColumn);
                gvSummary.Rows[i].Cells[1].HorizontalAlign = HorizontalAlign.Left;
                gvSummary.Rows[i].Cells[2].Style.Add("width", maxWidthForThirdColumn);
                gvSummary.Rows[i].Cells[2].HorizontalAlign = HorizontalAlign.Left;
                // gvSearch.Rows[i].Cells[0].CssClass = "locked";

            }
            gvSummary.Rows[0].Cells[0].Style.Add("vertical-align", "top");
            gvSummary.Rows[0].Cells[0].Style.Add("width", maxWidthForFirstColumn);
            gvSummary.Rows[0].Cells[0].HorizontalAlign = HorizontalAlign.Left;
            gvSummary.Rows[0].Cells[1].HorizontalAlign = HorizontalAlign.Left;
            gvSummary.Rows[0].Cells[1].Style.Add("width", maxWidthForSecondColumn);
            gvSummary.Rows[0].Cells[2].HorizontalAlign = HorizontalAlign.Left;
            gvSummary.Rows[0].Cells[2].Style.Add("width", maxWidthForThirdColumn);
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    protected void gvSummary_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ...
            if (e.Row.RowType == DataControlRowType.Header)
            {
                for (int i = 0; i < e.Row.Cells.Count; i++)
                {
                    e.Row.Cells[i].Text = "<div class=\"verticalHeaderText\" style='writing-mode:vertical-rl; transform: rotate(180deg)'>"
    ...
    protected void gvExportToPDF_DataBound(object sender, EventArgs e)
    {
        try
        {
            string maxWidthForFirstColumn = "150px";
            string maxWidthForSecondColumn = "150px";
            string maxWidthForThirdColumn = "350px";
            for (int i = gvExportToPDF.Rows.Count - 1; i > 0; i--)
            {
                GridViewRow row = gvExportToPDF.Rows[i];
                GridViewRow previousRow = gvExportToPDF.Rows[i - 1];
                for (int j = 0; j < row.Cells.Count; j++)
                {
                    if (row.Cells[0].Text == previousRow.Cells[0].Text)
                    {
                ...
                // e.Row.Cells[i].Text = "<div class=\"verticalHeaderText\" style='writing-mode:vertical-rl; transform: rotate(180deg)'>" + e.Row.Cells[i].Text + "</div>";
                // gvSearch.Rows[i].Cells[0].Style.Add("class", "verticalHeaderText");
                gvExportToPDF.Rows[i].Cells[0].Style.Add("vertical-align", "top");
                gvExportToPDF.Rows[i].Cells[0].Style.Add("width", maxWidthForFirstColumn);
                gvExportToPDF.Rows[i].Cells[0].HorizontalAlign = HorizontalAlign.Left;
                gvExportToPDF.Rows[i].Cells[1].Style.Add("width", maxWidthForSecondColumn);
                gvExportToPDF.Rows[i].Cells[1].HorizontalAlign = HorizontalAlign.Left;
                gvExportToPDF.Rows[i].Cells[2].Style.Add("width", maxWidthForThirdColumn);
                gvExportToPDF.Rows[i].Cells[2].HorizontalAlign = HorizontalAlign.Left;
                // gvSearch.Rows[i].Cells[0].CssClass = "locked";

            }
            gvExportToPDF.Rows[0].Cells[0].Style.Add("vertical-align", "top");
            gvExportToPDF.Rows[0].Cells[0].Style.Add("width", maxWidthForFirstColumn);
            gvExportToPDF.Rows[0].Cells[0].HorizontalAlign = HorizontalAlign.Left;
            gvExportToPDF.Rows[0].Cells[1].HorizontalAlign = HorizontalAlign.Left;
            gvExportToPDF.Rows[0].Cells[1].Style.Add("width", maxWidthForSecondColumn);
            gvExportToPDF.Rows[0].Cells[2].HorizontalAlign = HorizontalAlign.Left;
            gvExportToPDF.Rows[0].Cells[2].Style.Add("width", maxWidthForThirdColumn);
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    protected void gvExportToPDF_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            DataRowView dr = (DataRowView)e.Row.DataItem;
